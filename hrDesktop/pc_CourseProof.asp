	...
	If HR_Clng(UserYGDM) > 0 Then
		...
		Else
			...
			For m = 0 To Ubound(arrTmpID)
				Set rsTmp = Server.CreateObject("ADODB.RecordSet")
					rsTmp.Open("Select * From " & tSheetName & " Where VA1=" & HR_CLng(UserYGDM) & " And ID=" & HR_Clng(arrTmpID(m))), Conn, 1, 3
					If Not(rsTmp.BOF And rsTmp.EOF) Then
						tmpStuType = Trim(rsTmp("StudentType"))
						tYGXM = Trim(rsTmp("VA2"))
						tYGDM = HR_Clng(rsTmp("VA1"))
		...
		Set rsClass = Conn.Execute("Select * From HR_Class")
			If Not(rsClass.BOF And rsClass.EOF) Then
				Do While Not rsClass.EOF
					...
					If ChkTable(tSheetName) Then
						Conn.Execute("Update " & tSheetName & " Set State=1 Where State=0 And VA1=" & HR_Clng(UserYGDM))
					End If
					rsClass.MoveNext
				Loop
				ErrMsg = "您所有考核项目的课程业绩已确认提交！"
			Else
				ErrMsg = "没有考核项目！<br>"
			End If
		Set rsClass = Nothing
