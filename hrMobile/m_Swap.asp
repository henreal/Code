...
Dim scriptCtrl : SiteTitle = "调换课申请"
...
Sub MainBody()
	...
	tmpHtml = tmpHtml & "		.iconTit {padding-right:5px;font-size:20px;}" & vbCrlf
	...
	sqlTmp = "Select a.*,(Select ClassName From HR_CLass Where ClassID=a.ItemID) As ItemName,(Select YGXM From HR_Teacher Where YGDM=a.Replacer) As ReplacerXM From HR_Swap a Where a.YGDM=" & UserYGDM		'取课程SQL
	...
	Set rsTmp = Conn.Execute(sqlTmp)
		If Not(rsTmp.BOF And rsTmp.EOF) Then
			...
			Do While Not rsTmp.EOF
				...
				tCourseList = tCourseList & "	<a class=""weui-cell weui-cell_access"" href=""" & ParmPath & "Swap/Details.html?ItemID=" & rsTmp("ItemID") & "&ID=" & rsTmp("ID") & """>" & vbCrlf
				tCourseList = tCourseList & "		<div class=""weui-cell__hd iconTit""><i class=""hr-icon"">&#xf33c;</i></div>" & vbCrlf
				tCourseList = tCourseList & "		<div class=""weui-cell__bd weui-cell_primary viewMSG"" data-id=""" & rsTmp("CourseID") & """>" & vbCrlf
				tCourseList = tCourseList & "		<p>" & rsTmp("ItemName") & " " & FormatDate(Trim(rsTmp("VA4")), 4) & "<br>" & GetPeriodTime(rsTmp("VA11"), rsTmp("VA7"),0) & " 第" & rsTmp("VA7") & "节</p>"
				tCourseList = tCourseList & "		<p>课程名称：<span>" & rsTmp("VA8") & " " & rsTmp("VA11") & "</span></p>"
				tCourseList = tCourseList & "		<p>替课教师：<span>" & tReplacer & "</span></p>"
				tCourseList = tCourseList & "</div><div class=""weui-cell__ft"">" & tStep & "</div></a>"
				rsTmp.MoveNext
				i = i + 1
			Loop
		...
		Else
			tCourseList = "<h3 class=""hr-list-title""><i class=""hr-icon"">&#xe972;</i>您共有<b>0</b>条换课记录！</h3>"
			tCourseList = tCourseList & "<div class=""hr-noinfo""><h2><i class=""hr-icon"">&#xef61;</i></h2><h3>您暂时还没有调换课记录！</h3></div>" & vbCrlf
		End If
	Set rsTmp = Nothing

	Response.Write "<div class=""weui-cells"">" & tCourseList & "</div>" & vbCrlf
	Response.Write "<div class=""hr-float-btn"">" & vbCrlf
	Response.Write "	<a href=""" & ParmPath & "Swap/Edit.html?AddNew=True"" class=""addBtn""><i class=""hr-icon"">&#xf3c0;</i></a>" & vbCrlf
...
Sub Details()
	...
	Dim tVA2, tVA4, tCourse, tItem, tReason, tReplacerID, tReplacer, tPasser, tPasserID, tPassTime, tPasser1, tPasser2, tCourseDate, tPeriod, tPeriodTime, tPlace, tStuClass, NotModi, tProcess
	Dim tApplyID, tApplyer, tApplyerKS, tApplyerZW, tApplyerZC, tApplyTime
	Dim tReplacerKS, tReplacerZW, tReplacerZC, tReplacerTime
	NotModi = False
	...
	Set rsTmp = Conn.Execute(sqlTmp)
		If Not(rsTmp.BOF And rsTmp.EOF) Then
			...
			tPassTime = Trim(rsTmp("PassTime"))

			If HR_IsNull(rsTmp("ReplacerTime")) And HR_CLng(rsTmp("Process"))=0 Then
				tReplacer = tReplacer & "　[待确定]"
			ElseIf HR_IsNull(rsTmp("ReplacerTime")) And HR_CLng(rsTmp("Process"))=3 Then
				tReplacer = tReplacer & "　[已拒绝]"
			ElseIf HR_IsNull(rsTmp("ReplacerTime")) = False Then
				tReplacer = tReplacer & "　[已同意]" & FormatDate(rsTmp("ReplacerTime"), 10)
			End If

			tPasser = strGetTypeName("HR_Teacher", "YGXM", "YGDM", tPasserID)			'教研室主任
			If HR_IsNull(rsTmp("PassTime")) Then tPasser = tPasser & "<br>[未审]" Else tPasser = tPasser & "<br>[已审]" & FormatDate(rsTmp("PassTime"), 10)
			tPasser1 = strGetTypeName("HR_Teacher", "YGXM", "YGDM", HR_Clng(rsTmp("Passer1")))			'教学处
			If HR_IsNull(rsTmp("PassTime1")) Then tPasser1 = tPasser1 & "<br>[未审]" Else tPasser1 = tPasser1 & "<br>[已审]" & FormatDate(rsTmp("PassTime1"), 10)
			tPasser2 = strGetTypeName("HR_Teacher", "YGXM", "YGDM", HR_Clng(rsTmp("Passer2")))			'教辅
			If HR_IsNull(rsTmp("PassTime2")) Then tPasser2 = tPasser2 & "<br>[未审]" Else tPasser2 = tPasser2 & "<br>[已审]" & FormatDate(rsTmp("PassTime2"), 10)
			tReason = Trim(rsTmp("Reason"))
	...
	Response.Write "		<dl class=""hr-rows""><dt>替换教师：</dt><dd>" & tReplacer & "</dd></dl>" & vbCrlf
	...
	Response.Write "		<dl class=""hr-rows""><dt>教研室主任：</dt><dd>" & tPasser & "</dd></dl>" & vbCrlf
	Response.Write "		<dl class=""hr-rows""><dt>教学处：</dt><dd>" & tPasser1 & "</dd></dl>" & vbCrlf
	Response.Write "		<dl class=""hr-rows""><dt>教辅：</dt><dd>" & tPasser2 & "</dd></dl>" & vbCrlf
	Response.Write "		<div class=""hr-gap-20 hr-fix""></div>" & vbCrlf
	Response.Write "	</div>" & vbCrlf
	
	Response.Write "	<div class=""hr-shrink-x20""></div>" & vbCrlf
	If tProcess=0 And HR_IsNull(tReplacerTime) And UserYGDM = tApplyID Then
		...
		Response.Write "	</div>" & vbCrlf
	ElseIf HR_IsNull(tReplacerTime) And tProcess=0 And UserYGDM = tReplacerID Then		'替换老师
		Response.Write "	<div class=""hr-rows hr-editbtn"">" & vbCrlf
		Response.Write "		<em><i class=""hr-icon"">&#xea3f;</i></em>" & vbCrlf
		Response.Write "		<em><button type=""button"" name=""agree"" class=""agree"" data-id=""" & tmpID & """ data-pass=""true"">同意</button></em>" & vbCrlf
		Response.Write "		<em><button type=""button"" name=""agree"" class=""agree"" data-id=""" & tmpID & """ data-pass=""false"">拒绝</button></em>" & vbCrlf
		Response.Write "	</div>" & vbCrlf
	ElseIf UserYGDM = tPasserID Or SwapPass>0 Then		'教研主任审核或教学处及教辅
		Response.Write "	<div class=""hr-rows hr-editbtn"">" & vbCrlf
		Response.Write "		<em><i class=""hr-icon"">&#xea3f;</i></em>" & vbCrlf
		Response.Write "		<em class=""pass""><a href=""" & ParmPath & "SwapPass/Index.html?ID=" & tmpID & """ name=""edit"" class=""edit"" data-id=""" & tmpID & """>审核</a></em>" & vbCrlf
		Response.Write "	</div>" & vbCrlf
	End If
	Response.Write "</div>" & vbCrlf
	...
	tmpHtml = "<script type=""text/javascript"">" & vbCrlf
	tmpHtml = tmpHtml & "	$("".navMenu em"").on(""click"",function(){ $("".layerNav"").toggle(); });" & vbCrlf
	tmpHtml = tmpHtml & "	$("".navBack"").on(""click"",function(){ location.href=""" & ParmPath & "Swap/Index.html""; });" & vbCrlf
	tmpHtml = tmpHtml & "	$(""#EditForm"").on(""click"",function(){" & vbCrlf
	tmpHtml = tmpHtml & "		var swapid=$(this).data(""id"");" & vbCrlf
	tmpHtml = tmpHtml & "		location.href=""" & ParmPath & "Swap/Edit.html?Modify=True&ID=""+ swapid;" & vbCrlf
	tmpHtml = tmpHtml & "	});" & vbCrlf
	tmpHtml = tmpHtml & "	$(""#Delete"").on(""click"",function(){" & vbCrlf
	tmpHtml = tmpHtml & "		var swapid=$(this).data(""id"");" & vbCrlf
	tmpHtml = tmpHtml & "		$.confirm(""您确定要删除本次申请？"",function(){" & vbCrlf
	tmpHtml = tmpHtml & "			$.getJSON(""" & ParmPath & "Swap/Delete.html"",{ID:swapid},function(reData){" & vbCrlf
	tmpHtml = tmpHtml & "				$.toast(reData.reMessge, function(){location.href=""" & ParmPath & "Swap/Index.html""; });" & vbCrlf
	tmpHtml = tmpHtml & "			});" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf
	tmpHtml = tmpHtml & "	});" & vbCrlf
	tmpHtml = tmpHtml & "	$("".agree"").on(""click"",function(){" & vbCrlf
	tmpHtml = tmpHtml & "		var swapid=$(this).data(""id""), swappass=$(this).data(""pass"");" & vbCrlf
	tmpHtml = tmpHtml & "		var passtxt=""您确定要同意与" & tApplyer & "老师调换课吗？"";" & vbCrlf
	tmpHtml = tmpHtml & "		if(!swappass){ passtxt=""您确定要拒绝与" & tApplyer & "老师调换课吗？""; };" & vbCrlf
	tmpHtml = tmpHtml & "		$.confirm(passtxt,function(){" & vbCrlf
	tmpHtml = tmpHtml & "			$.getJSON(""" & ParmPath & "Swap/Agree.html"",{ID:swapid, Passed:swappass},function(reData){" & vbCrlf
	tmpHtml = tmpHtml & "				$.toast(reData.errmsg, function(){" & vbCrlf
	tmpHtml = tmpHtml & "					if(!reData.err){ location.href=""" & ParmPath & "Swap/ListAgree.html""; }" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "			});" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf
	tmpHtml = tmpHtml & "	});" & vbCrlf
...
Sub Edit()
	...
	Response.Write "	<div class=""old-box""><h3>课程信息：</h3><ul></ul></div>" & vbCrlf
	Response.Write "	<div class=""hr-gap-20 hr-gapbg""></div>" & vbCrlf
	...
	Response.Write "<div class=""weui-cells__title"">调换课理由：</div>" & vbCrlf
	Response.Write "<div class=""weui-cells weui-cells_form"">" & vbCrlf
	Response.Write "	<div class=""weui-cell"">" & vbCrlf
	Response.Write "		<div class=""weui-cell__bd""><textarea class=""weui-textarea"" name=""Reason"" id=""Reason"" placeholder=""请输入调换课理由"" rows=""5"">" & tReason & "</textarea></div>" & vbCrlf
	Response.Write "	</div>" & vbCrlf
	Response.Write "</div>" & vbCrlf
	...
	tmpHtml = tmpHtml & "	var laynav=""<li><a href=\""" & ParmPath & "Swap/Index.html\""><i class=\""hr-icon hr-icon-top\"">&#xe853;</i>我申请的调换课</a></li>"";" & vbCrlf
	tmpHtml = tmpHtml & "	laynav+=""<li><a href=\""" & ParmPath & "Swap/Alternate.html\""><i class=\""hr-icon hr-icon-top\"">&#xf2dd;</i>我的代授课</a></li>"";" & vbCrlf
	...
	tmpHtml = tmpHtml & "		$.post(""" & ParmPath & "Swap/SavePost.html"",$(""#EditForm"").serialize(), function(res){" & vbCrlf
	tmpHtml = tmpHtml & "			console.log(res);" & vbCrlf
	tmpHtml = tmpHtml & "			if(res.err){ $.toptip(res.errmsg, 'error'); }" & vbCrlf
	tmpHtml = tmpHtml & "			else{ $.toast(res.errmsg, function(){ location.href=""" & ParmPath & "Swap/Index.html""; }); }" & vbCrlf
	...
	tmpHtml = tmpHtml & "			$.getJSON(""" & ParmPath & "Swap/getCourse.html"",{ID:cid, ItemID:itemid}, function(redata){" & vbCrlf
	...
	tmpHtml = tmpHtml & "			$.get(""" & ParmPath & "Swap/getItemCourse.html"",{Item:tid}, function(strForm){" & vbCrlf
...
Sub SavePost()
	...
	ErrMsg = "申请提交成功！"
	'发送提醒
	Dim url1 : url1 = SiteUrl & "/Touch/Swap/Details.html?ItemID=" & tItemID & "&ID=" & tmpID
	Call SentWechatMSG_QYCard(UserYGDM, UserYGXM & "：您已申请与" & tReplacerName & "老师调换课程", url1, UserYGXM & "老师：您申请与" & tReplacerName & "老师调换课程已提交成功，等待教研主任" & Trim(Request("Passer")) & "审核。<br>理由：" & tReason & "<br>申请时间：" & FormatDate(Now(), 1))	'发送给自己
	Call SentWechatMSG_QYCard(tReplacer, UserYGXM & " 申请与您调换课程，申请正在审核中", url1, UserYGXM & "老师申请与您调换课程。<br>理由：" & tReason & "<br>申请时间：" & FormatDate(Now(), 1))	'发送给替课老师
...
Sub Alternate()			'代课记录
	...
	Dim tCourseList, tStep, tPeriodTime, tApplyer
	...
	Set rsTmp = Conn.Execute(sqlTmp)
		If Not(rsTmp.BOF And rsTmp.EOF) Then
			...
			Do While Not rsTmp.EOF
				...
				tCourseList = tCourseList & "		<div class=""weui-cell__hd iconTit""><i class=""hr-icon"">&#xf33c;</i></div>" & vbCrlf
				tCourseList = tCourseList & "		<div class=""weui-cell__bd weui-cell_primary viewMSG"" data-id=""" & rsTmp("CourseID") & """>" & vbCrlf
				tCourseList = tCourseList & "		<p>" & rsTmp("ItemName") & " " & FormatDate(ConvertNumDate(rsTmp("VA4")), 4) & "<br>" & GetPeriodTime(rsTmp("VA11"), rsTmp("VA7"),0) & " 第" & rsTmp("VA7") & "节</p>"
				tCourseList = tCourseList & "		<p>课程名称：<span>" & rsTmp("VA8") & " " & rsTmp("VA11") & "</span></p>"
				tCourseList = tCourseList & "		<p>申请教师：<span>" & tApplyer & "</span></p>"
				tCourseList = tCourseList & "</div><div class=""weui-cell__ft"">" & tStep & "</div></a>"
				rsTmp.MoveNext
				i = i + 1
			Loop
		...
		Else
			tCourseList = "<h3 class=""hr-list-title""><i class=""hr-icon"">&#xe972;</i>您共有<b>0</b>条代课记录！</h3>"
...
Sub Agree()
	Dim tmpID : tmpID = HR_Clng(Request("ID"))
	Dim tPassed : tPassed = HR_CBool(Request("Passed"))
	...
	Set rsTmp = Conn.Execute(sqlTmp)
		If Not(rsTmp.BOF And rsTmp.EOF) Then
			...
			Else
				Dim url1 : url1 = SiteUrl & "/Touch/Swap/Details.html?ItemID=" & rsTmp("ItemID") & "&ID=" & tmpID
				...
				If tPassed Then
					ErrMsg = UserYGXM & "老师同意了调换课程，操作时间：" & Now()
					Conn.Execute("Update HR_Swap Set ReplacerTime='" & Now() & "' Where ID=" & tmpID)
					strTmp = "{""err"":false,""errcode"":0,icon:1,""errmsg"":""您已经同意了调换课程！""}"
					Call SentWechatMSG_QYCard(UserYGDM, UserYGXM & "：您已同意与" & tApplyer & "老师调换课程", url1, UserYGXM & "老师：您已同意与" & tApplyer & "老师调换课程，等待教研主任" & tPasser & "审核。<br>操作时间：" & FormatDate(Now(), 1))	'发送给自己
					Call SentWechatMSG_QYCard(HR_CLng(rsTmp("YGDM")), UserYGXM & "已同意与您调换课程", url1, tApplyer & "老师：" & UserYGXM  & "已同意与您调换课程，等待教研主任" & tPasser & "审核。<br>操作时间：" & FormatDate(Now(), 1))	'发送给申请人
				Else
					ErrMsg = UserYGXM & "老师拒绝调换该课程，操作时间：" & Now()
					Conn.Execute("Update HR_Swap Set Process=3,Explain='" & ErrMsg & "' Where ID=" & tmpID)
					strTmp = "{""err"":false,""errcode"":0,icon:1,""errmsg"":""您已经同意了调换课程！""}"
					Call SentWechatMSG_QYCard(UserYGDM, UserYGXM & "：您已拒绝与" & tApplyer & "老师调换课程", url1, UserYGXM & "老师：您已拒绝与" & tApplyer & "老师调换课程。<br>操作时间：" & FormatDate(Now(), 1))	'发送给自己
					Call SentWechatMSG_QYCard(HR_CLng(rsTmp("YGDM")), UserYGXM & "拒绝与您调换课程", url1, tApplyer & "老师：" & UserYGXM  & "已拒绝与您调换课程。<br>操作时间：" & FormatDate(Now(), 1))	'发送给申请人
				End If
			End If
		End If
	Set rsTmp = Nothing
