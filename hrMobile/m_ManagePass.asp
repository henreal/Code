...
Sub MainBody()
	...
	strHtml = strHtml & "		body {background-color:#f2f2f2;} .hr-header {background-color:#f30;}" & vbCrlf
	...
	Set rs = Conn.Execute("Select ClassID From HR_Class Where ModuleID=1001 And Child=0")
		If Not(rs.BOF And rs.EOF) Then
			...
			Do While Not rs.EOF
				...
				sql = sql & "select count(1) as CNT From HR_Sheet_" & rs("ClassID") & " Where Passed=0 And scYear=" & DefYear
				rs.MoveNext
				i = i + 1
			Loop
		End If
	...
	Response.Write "		<div class=""weui-form-preview__item""><label class=""weui-form-preview__label"">未审核课程</label><em class=""weui-form-preview__value"">" & HR_Clng(TotalNum) & "</em></div>"  & vbCrlf
	...
	Set rsMenu = Conn.Execute(sqlMenu)
		If Not(rsMenu.BOF And rsMenu.EOF) Then
			...
			Do While Not rsMenu.EOF
				...
				Else
					Set rsCount = Conn.Execute("Select count(1) as CNT From HR_Sheet_" & rsMenu("ClassID") & " Where Passed=0 And scYear=" & DefYear)
						iRetreat = HR_Clng(rsCount(0))
					Set rsCount = Nothing
					strMenu = strMenu & " class=""hr-navmenu"" href=""" & ParmPath & "ManagePass/List.html?ItemID=" & rsMenu("ClassID") & """><em class=""title"">" & fIcon & fItemName & "</em><em>" & iRetreat & "</em><em class=""more""><i class=""hr-icon"">&#xef91;</i></em></a>"
				End If
...
Sub List()
	...
	Set rs = Conn.Execute("Select Count(1) From " & tSheetName & " Where Passed=0 And scYear=" & DefYear)
		TotalNum = HR_Clng(rs(0))
	Set rs = Nothing

	sqlList = "Select Top 300 a.*,(Select KSMC From HR_Teacher Where YGDM=a.VA1) As KSMC1 From " & tSheetName & " a Where Passed=0 And scYear=" & DefYear
	...
	Set rsList = Server.CreateObject("ADODB.RecordSet")
		...
		If Not(rsList.BOF And rsList.EOF) Then
			...
			Loop
		Else
			strList = "<div class=""nolist""><em><i class=""hr-icon"">&#xef61;</i></em><span>没有未审课程</span></div>"
		End If
	Set rsList = Nothing
	...
	Response.Write "<div class=""weui-form-preview"">" & vbCrlf
	Response.Write "	<div class=""weui-form-preview__hd"">" & vbCrlf
	Response.Write "		<div class=""weui-form-preview__item""><label class=""weui-form-preview__label"">未审核课程数</label><em class=""weui-form-preview__value"">" & HR_Clng(TotalNum) & "</em></div>"  & vbCrlf
	Response.Write "	</div>" & vbCrlf
	Response.Write "</div>" & vbCrlf
	'Response.Write "<div class=""hr-gap-20 hr-fix""></div>" & vbCrlf
	...
	Response.Write "<div class=""hr-shrink-x10""></div>" & vbCrlf
	strHtml = "<script type=""text/javascript"">" & vbCrlf
	strHtml = strHtml & "	$("".navBack"").on(""click"",function(){ location.href=""" & ParmPath & "ManagePass/Index.html""; });" & vbCrlf
	strHtml = strHtml & "	$("".navMenu em"").on(""click"",function(){ $("".layerNav"").toggle(); });" & vbCrlf
	strHtml = strHtml & "</script>" & vbCrlf
	strFootHtml = Replace(strFootHtml, "[@FootScript]", strHtml)
	Response.Write ReplaceCommonLabel(strFootHtml)
