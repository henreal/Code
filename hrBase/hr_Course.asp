...
Select Case Action
	...
	Case "Passed" Call Passed()
	Case "unPass" Call AntiPass()		'取消审核
	Case "Delete" Call Delete()
	Case "Import" Call InStep1()
	Case "InStep2" Call InStep2()
	Case "InStep3" Call InStep3()

	Case "levelData" Call levelData()
	Case "CampusData" Call CampusData()
	
	Case "ExcelTemp" Call ExcelTemp()
	Case "SaveExcelName" Call SaveExcelName()
	Case "ViewItem" Call ViewItem()

	Case "SwitchLock" Call SwitchLock()

	Case Else Response.Write GetErrBody(0)
End Select

Sub MainBody()
	Dim tCourse, rsList, tChild, tTemplate, tSheetName, tStuType, arrStuType, tFieldHead, arrHead, tFieldLen
	Dim tCourseID : tCourseID = HR_Clng(Request("ItemID"))
	Dim tSearchWord : tSearchWord = Trim(ReplaceBadChar(Request("SearchWord")))
	Dim tKSDM : tKSDM = Trim(ReplaceBadChar(Request("KSMC")))
	Dim soYear : soYear = HR_Clng(Request("soYear"))
	If soYear < 2000 Then soYear = DefYear	'如果学年不正确，取系统默认学年

	Set rsTmp = Conn.Execute("Select * From HR_Class Where ClassID=" & tCourseID)
		...
		Else
			...
			tSheetName = "HR_Sheet_" & tCourseID
		End If
	Set rsTmp = Nothing
	...
	tmpHtml = tmpHtml & "		.onekey {background-color:#754;}" & vbCrlf
	tmpHtml = tmpHtml & "		.batchbtn {background-color:#672;}" & vbCrlf
	tmpHtml = tmpHtml & "		.LockBar .layui-form-switch {margin-top:0px;}" & vbCrlf
	tmpHtml = tmpHtml & "		.btnAffirm {background:#158;} .oneAffirm {background:#809;} .reSum {color:#f30;}" & vbCrlf
	...
	tmpHtml = tmpHtml & "		.layui-table-init .layui-icon {color:#F60; margin:-40px 0 0 -40px;font-size:80px;}" & vbCrlf		'分页时提示符
	...
	tmpHtml = "<a href=""" & ParmPath & "Course.html?ItemID=" & tCourseID & """>" & SiteTitle & "</a><a><cite>" & tCourse & "</cite></a>"
	...
	Response.Write "	<fieldset class=""layui-elem-field layui-field-title"" style=""margin-top:1px;""><legend>" & tCourse & "</legend></fieldset>" & vbCrlf
	If tChild = 0 Then
		...
		Response.Write "<div class=""layui-inline"" style=""width:100px""><input class=""layui-input"" name=""soTeacher"" value=""" & tSearchWord & """ id=""soTeacher"" placeholder=""员工姓名/工号"" autocomplete=""off"" /></div>" & vbCrlf
		Response.Write "<div class=""layui-inline""><select name=""KSMC"" id=""KSMC"" lay-search=""""><option value="""">选择/搜索科室名称</option>" & GetDeptOption(0, tKSDM, 0) & "</select></div>" & vbCrlf
		...
		Response.Write "<div class=""layui-inline"" style=""width:130px""><select name=""soSort"" id=""soSort""><option value="""">选择排序方式</option><option value=""importTimeUP"">上传时间正序↑</option><option value=""importTimeDown"">上传时间倒序↓</option>"
		Response.Write "<option value=""xhUP"">序号正序↑</option><option value=""xhDown"">序号倒序↓</option></select></div>" & vbCrlf
		Response.Write "<div class=""layui-inline"" style=""width:120px""><select name=""upuser"" id=""upuser""><option value="""">选择上传者</option>" & GetUploadUerOption() & "</select></div>" & vbCrlf
		...
		If tStuType <> "" Then
			Response.Write "		<div class=""layui-inline"" style=""width:130px""><select name=""StudentType"" id=""StudentType"" lay-verify=""required""><option value="""">选择学生类别</option>" & vbCrlf
		...
		End If
		Response.Write "<div class=""layui-inline""><input type=""checkbox"" name=""IsMeload"" id=""IsMeload"" value=""true"" title=""我上传的""></div>" & vbCrlf
		Response.Write "<div class=""layui-inline""><input type=""checkbox"" name=""IsPass"" id=""IsPass"" value=""true"" title=""未审核""></div>" & vbCrlf
		Response.Write "<div class=""layui-inline""><input type=""checkbox"" name=""IsAffirm"" id=""IsAffirm"" value=""true"" title=""已确认""></div>" & vbCrlf
		Response.Write "<div class=""layui-inline""><input type=""checkbox"" name=""IsRetreat"" id=""IsRetreat"" value=""true"" title=""已退回""></div>" & vbCrlf

		Response.Write "		<div class=""layui-inline searchBtn""><button type=""button"" class=""layui-btn layui-bg-cyan"" data-type=""reload"" id=""SearchBtn""><i class=""layui-icon"">&#xe615;</i>搜索</button></div>" & vbCrlf
		Response.Write "		<div class=""layui-btn-group searchBtn"">" & vbCrlf
		'If IsAdd Then
		'	Response.Write "			<button type=""button"" class=""layui-btn layui-btn-normal"" data-type=""addNew"" id=""addNew"" title=""新增课程""><i class=""layui-icon"">&#xe654;</i></button>" & vbCrlf
		'Else
		'	Response.Write "			<button type=""button"" class=""layui-btn layui-btn-normal layui-btn-disabled"" title=""新增课程关闭""><i class=""layui-icon"">&#xe654;</i></button>" & vbCrlf
		'End If
		'Response.Write "			<button type=""button"" class=""layui-btn layui-btn-normal"" data-type=""refresh"" id=""refresh"" title=""刷新""><i class=""hr-icon"">&#xebbb;</i></button>" & vbCrlf
		
		Response.Write "			<button type=""button"" class=""layui-btn batchbtn"" data-type=""pass"" id=""BatchPass"" title=""批量审核""><i class=""hr-icon"">&#xef8a;</i>审核提交</button>" & vbCrlf
		Response.Write "			<button type=""button"" class=""layui-btn batchbtn"" data-type=""batchBack"" id=""BatchBack"" title=""批量退回""><i class=""hr-icon"">&#xeeb9;</i>退回</button>" & vbCrlf
		If UserRank > 1 Then Response.Write "			<button type=""button"" class=""layui-btn btnAffirm"" data-type=""BatchUnPass"" id=""BatchUnPass"" title=""批量反审核""><i class=""hr-icon"">&#xe3e0;</i>反审核</button>" & vbCrlf
		
		'Response.Write "			<button type=""button"" class=""layui-btn layui-bg-cyan"" data-type=""batchaffirm"" id=""BatchAffirm"" title=""确认提交""><i class=""hr-icon"">&#xebc5;</i>确认提交</button>" & vbCrlf
		'Response.Write "			<button type=""button"" class=""layui-btn oneAffirm"" data-type=""oneAffirm"" id=""oneAffirm"" title=""一键确认""><i class=""hr-icon"">&#xf046;</i>一键确认</button>" & vbCrlf
		Response.Write "		</div>" & vbCrlf
		If UserRank > 1 Then
			Response.Write "			<div class=""layui-btn-group searchBtn""><button type=""button"" class=""layui-btn layui-bg-red"" data-type=""lockon"" id=""LockOn"" title=""一键审核"">一键审核</button>" & vbCrlf
			Response.Write "			<button type=""button"" class=""layui-btn layui-bg-green"" data-type=""lockoff"" id=""LockOff"" title=""一键反审核"">一键反审核</button>" & vbCrlf
			'Response.Write "			<button type=""button"" class=""layui-btn layui-btn-normal"" data-type=""updateKPI"" id=""UpdateKPI"" title=""更新本项目报表"">更新报表</button>" & vbCrlf
			Response.Write "			<button type=""button"" class=""layui-btn layui-btn-normal"" data-type=""exportall"" id=""ExportAll"" title=""导出全表"">导出全表</button>" & vbCrlf
			Response.Write "			</div>" & vbCrlf
		End If
		Response.Write "<div class=""layui-inline""><span class=""layui-btn layui-btn-primary"">总" & arrHead(3) & "：<b class=""reSum"">0</b></span></div>" & vbCrlf
		Response.Write "	<input type=""hidden"" name=""CourseID"" value=""" & tCourseID & """>" & vbCrlf
		Response.Write "	</form>" & vbCrlf
	
		Response.Write "	<table class=""layui-table"" lay-data=""{url:'" & ParmPath & "Course/AllData.html?CourseID=" & tCourseID & "&SearchWord=" & tSearchWord & "',height:'full-220',page:true,toolbar: '#tabbtn',limit:20,limits:[10,20,50,100,200,500,1000,2000],text:{none:'暂时未找到相关课程'},id:'TableList'}"" lay-filter=""TableList"">"
		...
		Response.Write "			<th lay-data=""{field:'VA3',align:'right',sort:true, width:70,templet:'#tplUnit'}"">" & arrHead(3) & "</th>" & vbCrlf
		...
		Response.Write "			<th lay-data=""{field:'Affirm',unresize:true,align:'center',width:60,templet:'#AffirmTpl'}"">确认</th>" & vbCrlf
		Response.Write "			<th lay-data=""{field:'Retreat',unresize:true,sort:true,align:'center',width:60,templet:'#BackTpl'}"">退回</th>" & vbCrlf
		...
		If IsImport Then		'导入
			Response.Write "				<button type=""button"" class=""layui-btn layui-btn-normal"" lay-event=""import"" id=""import"" title=""课程导入"">我的导入</button>" & vbCrlf
		Else
			Response.Write "				<button type=""button"" class=""layui-btn layui-btn-normal layui-btn-disabled"" title=""课程导入关闭""><i class=""hr-icon"">&#xecb8;</i>导入</button>" & vbCrlf
		End If
		...
		Response.Write "	<script type=""text/html"" id=""AffirmTpl"">" & vbCrlf		'确认模板
		Response.Write "		<input type=""checkbox"" name=""Affirm"" value=""{{d.isAffirm}}"" lay-skin=""primary"" lay-filter=""affirmchk"" disabled {{ d.Affirm===""已确认"" ? 'checked':'' }}>" & vbCrlf
		Response.Write "	</script>" & vbCrlf
		Response.Write "	<script type=""text/html"" id=""BackTpl"">" & vbCrlf		'退回模板
		Response.Write "		<input type=""checkbox"" name=""back"" value=""{{d.isBack}}"" lay-skin=""primary"" lay-filter=""affirmchk"" disabled {{ d.Retreat===""是"" ? 'checked':'' }}>" & vbCrlf
		Response.Write "	</script>" & vbCrlf
		...
		Response.Write "			{{#  } }}" & vbCrlf
		Response.Write "			{{#  if(d.isPass===true){ }}" & vbCrlf
		Response.Write "				{{#  if(d.Passed===true){ }}" & vbCrlf
		Response.Write "				<a class=""layui-btn layui-btn-sm"" lay-event=""verify"" title=""反审核""><i class=""hr-icon"">&#xef8a;</i></a>" & vbCrlf
		Response.Write "				{{#  }else{ }}" & vbCrlf
		Response.Write "				<a class=""layui-btn layui-btn-sm {{=d.passStyle }}"" lay-event=""verify"" title=""审核""><i class=""hr-icon"">&#xef8a;</i></a>" & vbCrlf
		Response.Write "				{{#  } }}" & vbCrlf
		Response.Write "			{{#  }else{ }}" & vbCrlf
		Response.Write "				<a class=""layui-btn layui-btn-sm layui-btn-disabled"" title=""无审核权""><i class=""hr-icon"">&#xef8a;</i></a>" & vbCrlf
		Response.Write "			{{#  } }}" & vbCrlf

		Response.Write "			{{#  if(d.isBack===true){ }}" & vbCrlf
		Response.Write "				<a class=""layui-btn layui-btn-sm batchbtn"" lay-event=""backModi"" title=""退回修改""><i class=""hr-icon"">&#xeeb9;</i></a>" & vbCrlf
		Response.Write "			{{#  }else{ }}" & vbCrlf
		Response.Write "				<a class=""layui-btn layui-btn-sm layui-btn-disabled"" title=""退回修改""><i class=""hr-icon"">&#xeeb9;</i></a>" & vbCrlf
		Response.Write "			{{#  } }}" & vbCrlf
	...
	Else
		Response.Write "<div class=""hr-workZones hr-shrink-x10"">" & vbCrlf
		Response.Write "	<div class=""hr-grids panBox"">" & vbCrlf
		Dim tIntro
		Set rsTmp = Conn.Execute("Select * From HR_Class Where ParentID=" & tCourseID & " Order By OrderID")
			If Not(rsTmp.BOF And rsTmp.EOF) Then
				Do While Not rsTmp.EOF
					tIntro = Trim(rsTmp("Tips"))
					Response.Write "<div class=""panItem""><div class=""panBorder""><em><i class=""hr-icon hr-icon-top"">&#xe99f;</i>" & Trim(rsTmp("ClassName")) & "</em><span>" & tIntro & "</span>"
					Response.Write "<div class=""layui-btn-group""><button class=""layui-btn layui-btn-sm layui-bg-cyan AddCourse"" data-id=""" & HR_Clng(rsTmp("ClassID")) & """ data-type=""add"" title=""添加课程""><i class=""hr-icon"">&#xf369;</i></button>"
					Response.Write "<button class=""layui-btn layui-btn-sm ViewItem"" data-id=""" & HR_Clng(rsTmp("ClassID")) & """ data-type=""list"" title=""查看详情""><i class=""hr-icon"">&#xe99c;</i></button></div>"
	...
	tmpHtml = tmpHtml & "					url:'" & ParmPath & "Course/AllData.html', where:postStr" & vbCrlf
	...
	tmpHtml = tmpHtml & "			}else if(btnEvent==""import""){" & vbCrlf
	tmpHtml = tmpHtml & "				location.href=""" & ParmPath & "CourseImport.html?ItemID=" & tCourseID & """;" & vbCrlf
	'tmpHtml = tmpHtml & "				layer.open({type:2,content:""" & ParmPath & "Course/Import.html?ItemID=" & tCourseID & """,title:[""导入课程数据"",""font-size:16""]" & vbCrlf
	'tmpHtml = tmpHtml & "					,area:[""700px"", ""430px""], maxmin:true" & vbCrlf
	'tmpHtml = tmpHtml & "					,cancel: function(index, layero){ table.reload(""TableList""); }" & vbCrlf
	'tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "			}else if(btnEvent==""BatchUnPass""){" & vbCrlf		'批量反审核
	tmpHtml = tmpHtml & "				var checkStatus = table.checkStatus(""TableList"");" & vbCrlf
	tmpHtml = tmpHtml & "				if(checkStatus.data.length==0){layer.tips(""请选择您要反审核的课程记录！"",""#BatchUnPass"",{tips: [1, ""#F52""]});return false;}" & vbCrlf
	tmpHtml = tmpHtml & "				layer.confirm(""您确定要反审核 “"" + checkStatus.data.length + ""”条课程业绩？"",{icon: 3},function(index){" & vbCrlf
	tmpHtml = tmpHtml & "					var arrID = """", loadMsg = layer.load(1,{shade:[0.1, ""#000""]});" & vbCrlf
	...
	tmpHtml = tmpHtml & "					}" & vbCrlf
	tmpHtml = tmpHtml & "					$.post(""" & ParmPath & "Course/unPass.html"",{ItemID:" & tCourseID & ",ID:arrID}, function(strForm){" & vbCrlf
	tmpHtml = tmpHtml & "						layer.msg(strForm.reMessge,{btn:""关闭"",time:0},function(){ table.reload(""TableList""); layer.close(loadMsg); });" & vbCrlf
	tmpHtml = tmpHtml & "					},""json"");" & vbCrlf
	tmpHtml = tmpHtml & "					return false;" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "			}else if(btnEvent==""addNew""){" & vbCrlf
	tmpHtml = tmpHtml & "				layer.open({type:2,content:""" & ParmPath & "Course/AddNew.html?ItemID=" & tCourseID & """,title:[""添加课程"",""font-size:16""],area:[""760px"", ""82%""],maxmin:true});" & vbCrlf
	...
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "CourseProof/Affirm.html"",{ItemID:" & tCourseID & ", ID:arrID}, function(reData){" & vbCrlf
	...
	tmpHtml = tmpHtml & "					});" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "			}else if(btnEvent==""pass""){" & vbCrlf
	tmpHtml = tmpHtml & "				var checkStatus = table.checkStatus(""TableList"");" & vbCrlf
	tmpHtml = tmpHtml & "				if(checkStatus.data.length==0){layer.tips(""请选择您要审核的课程记录！"",""#BatchPass"",{tips: [3, ""#F30""]});return false;}" & vbCrlf
	tmpHtml = tmpHtml & "				layer.confirm(""您确定要审核 “"" + checkStatus.data.length + ""”条课程业绩？<br>审核后将不能修改。"",{icon: 3, title:""重要提示""},function(index){" & vbCrlf
	tmpHtml = tmpHtml & "					var arrID = """", loadMsg = layer.load(1,{shade:[0.1, ""#000""]});" & vbCrlf
	tmpHtml = tmpHtml & "					for(var i=0;i<checkStatus.data.length;i++){" & vbCrlf
	tmpHtml = tmpHtml & "						if(i > 0){arrID = arrID + "",""}" & vbCrlf
	tmpHtml = tmpHtml & "						arrID = arrID + checkStatus.data[i].ID;" & vbCrlf
	tmpHtml = tmpHtml & "					}" & vbCrlf
	tmpHtml = tmpHtml & "					$.post(""" & ParmPath & "Course/Passed.html"",{ItemID:" & tCourseID & ",ID:arrID}, function(strForm){" & vbCrlf
	tmpHtml = tmpHtml & "						layer.msg(strForm.reMessge,{btn:""关闭"",time:0},function(){ table.reload(""TableList""); layer.close(loadMsg); });" & vbCrlf
	tmpHtml = tmpHtml & "					},""json"");" & vbCrlf
	tmpHtml = tmpHtml & "					return false;" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "			}else if(btnEvent==""batchBack""){" & vbCrlf
	tmpHtml = tmpHtml & "				var checkStatus = table.checkStatus(""TableList"");" & vbCrlf
	tmpHtml = tmpHtml & "				if(checkStatus.data.length==0){layer.tips(""请选择您要退回的课程记录！"",""#BatchBack"",{tips: [3, ""#F30""]});return false;}" & vbCrlf
	tmpHtml = tmpHtml & "				var arrID = [];" & vbCrlf
	tmpHtml = tmpHtml & "				for(var i=0;i<checkStatus.data.length;i++){arrID.push(checkStatus.data[i].ID);}" & vbCrlf
	...
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "CourseProof/backSave.html"",{ItemID:" & tCourseID & ", ID:arrID.join(), userid:" & UserID & ", Explain:tExplain}, function(reData){" & vbCrlf
	tmpHtml = tmpHtml & "						layer.msg(reData.reMessge,{icon:0,btn:""关闭"",time:0},function(){layer.closeAll();table.reload(""TableList"");});" & vbCrlf
	tmpHtml = tmpHtml & "					});" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "			}else if(btnEvent==""1knopass""){" & vbCrlf
	tmpHtml = tmpHtml & "				layer.confirm(""所有 " & tCourse & " 的课程业绩将取消审核，您确定继续操作？"",{icon: 3, title:""重要提示""},function(index){" & vbCrlf
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "Course/1knopass.html"",{ItemID:" & tCourseID & "}, function(strForm){" & vbCrlf
	tmpHtml = tmpHtml & "						layer.msg(strForm.reMessge,{btn:""关闭""},function(){table.reload(""TableList"");});" & vbCrlf
	tmpHtml = tmpHtml & "					});" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	...
	tmpHtml = tmpHtml & "						arrID = arrID + checkStatus.data[i].ID;" & vbCrlf
	tmpHtml = tmpHtml & "					}" & vbCrlf
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "Course/Delete.html"",{ItemID:" & tCourseID & ",ID:arrID}, function(strForm){" & vbCrlf
	tmpHtml = tmpHtml & "						layer.msg(strForm.reMessge,{btn:""关闭"",time:0},function(){table.reload(""TableList"");});" & vbCrlf
	tmpHtml = tmpHtml & "					});" & vbCrlf
	tmpHtml = tmpHtml & "					return false;" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "			}else if(btnEvent==""exportall""){" & vbCrlf		'导出全表为Excel
	tmpHtml = tmpHtml & "				layer.open({type:2, content:""" & ParmPath & "Export/ExportAll.html?ItemID=" & tCourseID & """, title:[""导出全表数据"",""font-size:16""],area:[""560px"", ""300px""],moveOut:true});" & vbCrlf
	tmpHtml = tmpHtml & "			}" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf
	tmpHtml = tmpHtml & "		$("".AddCourse"").on(""click"", function(){" & vbCrlf
	tmpHtml = tmpHtml & "			var tID = $(this).data(""id"");location.href=""" & ParmPath & "Course.html?ItemID="" + tID;" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf

	tmpHtml = tmpHtml & "		$(""#LockOff"").on(""click"", function(data){" & vbCrlf		'解锁-一键反审核
	tmpHtml = tmpHtml & "			layer.confirm(""本次操作将会反审核所有课程，您确定？<br />教师业绩也将同步更新。"",{icon: 3, title:""警告""},function(index){" & vbCrlf
	tmpHtml = tmpHtml & "				var msgLock = layer.msg(""系统正在 反审核 本考核项目的业绩数据…<br>根据数据多少，可能会花很长时间。<br><b style='color:#f00'>请不要关闭浏览器或离开本页</b>，否则业绩报表将不准确；<br>请您耐心等待！"", {icon: 0,offset:'220px',time:0}); ;" & vbCrlf
	tmpHtml = tmpHtml & "				var loadLock = layer.load();" & vbCrlf
	tmpHtml = tmpHtml & "				$.getJSON(""" & ParmPath & "Course/SwitchLock.html"",{ItemID:" & tCourseID & ", Locked:false}, function(strForm){" & vbCrlf
	tmpHtml = tmpHtml & "					layer.msg(strForm.reMessge,{icon:strForm.icon});" & vbCrlf
	tmpHtml = tmpHtml & "					layer.close(loadLock); " & vbCrlf			'关闭提示
	'tmpHtml = tmpHtml & "					if(strForm.Return){ table.reload(""TableList""); layer.close(msgLock);}" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "			});" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf
	tmpHtml = tmpHtml & "		$(""#LockOn"").on(""click"", function(data){" & vbCrlf		'锁定一键审核
	tmpHtml = tmpHtml & "			var msgLock = layer.msg(""系统正在 审核 本考核项目的业绩数据…<br>根据数据多少，可能会花很长时间。<br><b style='color:#f00'>请不要关闭浏览器或离开本页</b>，否则业绩报表将不准确；<br>请您耐心等待！"", {icon: 0,offset:'220px',time:0}); ;" & vbCrlf
	tmpHtml = tmpHtml & "			var loadLock = layer.load();" & vbCrlf
	tmpHtml = tmpHtml & "			$.getJSON(""" & ParmPath & "Course/SwitchLock.html"",{ItemID:" & tCourseID & ", Locked:true}, function(strForm){" & vbCrlf
	tmpHtml = tmpHtml & "				layer.tips(strForm.reMessge, data.othis, {tips:1});" & vbCrlf
	tmpHtml = tmpHtml & "				layer.close(loadLock); layer.close(msgLock); table.reload(""TableList"");" & vbCrlf
	tmpHtml = tmpHtml & "			});" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf


	tmpHtml = tmpHtml & "		$("".ViewItem"").on(""click"", function(){" & vbCrlf
	tmpHtml = tmpHtml & "			var tID = $(this).data(""id"");" & vbCrlf
	tmpHtml = tmpHtml & "			layer.open({type:1,id:""ViewWin"",content:"""", title:[""查看项目详情"",""font-size:16""], area:[""730px"", ""82%""],maxmin:true});" & vbCrlf
	tmpHtml = tmpHtml & "			$.get(""" & ParmPath & "Course/ViewItem.html"",{ItemID:tID}, function(strForm){" & vbCrlf
	tmpHtml = tmpHtml & "				$(""#ViewWin"").html(strForm);" & vbCrlf
	tmpHtml = tmpHtml & "			});" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf
	...
	tmpHtml = tmpHtml & "					var tExplain = $(""#Explain"").val();" & vbCrlf
	tmpHtml = tmpHtml & "					if(tExplain == """"){layer.msg(""请输入退回理由"",{icon:0,btn:""关闭""});return false;}" & vbCrlf
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "CourseProof/backSave.html"",{ItemID:" & tCourseID & ", ID:data.ID, ygdm:data.VA1, userid:" & UserID & ", Explain:tExplain}, function(reData){" & vbCrlf
	tmpHtml = tmpHtml & "						layer.msg(reData.reMessge,{icon:0,btn:""关闭"",time:0},function(){layer.closeAll();table.reload(""TableList"");});" & vbCrlf
	tmpHtml = tmpHtml & "					});" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	...
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "CourseProof/Affirm.html"",{ItemID:" & tCourseID & ", ID:data.ID}, function(reData){" & vbCrlf
	tmpHtml = tmpHtml & "						layer.alert(reData.reMessge,{icon:1,title:""确认提交""});" & vbCrlf
	tmpHtml = tmpHtml & "					});" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "			}else if(obj.event === ""verify""){" & vbCrlf
	tmpHtml = tmpHtml & "				var tipsText = ""您正准备审核该课程，继续？？"";" & vbCrlf
	tmpHtml = tmpHtml & "				if(data.Passed){ tipsText =""您将确认要反审核该课程？<br>反审核后可以进行修改等操作。""; }" & vbCrlf
	tmpHtml = tmpHtml & "				layer.confirm("""" + tipsText + """", {icon:3,title: ""系统提醒""}, function(index){" & vbCrlf
	...
	tmpHtml = tmpHtml & "					layer.open({type:2, content:'" & ParmPath & "Course/AddNew.html?ItemID=" & tCourseID & "',title:[""添加新课程"",""font-size:16""],area:[""760px"",""90%""],maxmin:true});" & vbCrlf
	tmpHtml = tmpHtml & "					break;" & vbCrlf
	tmpHtml = tmpHtml & "				case ""import"":" & vbCrlf
	tmpHtml = tmpHtml & "					location.href=""" & ParmPath & "CourseImport.html?ItemID=" & tCourseID & """;" & vbCrlf
	'tmpHtml = tmpHtml & "					layer.open({type:2,content:""" & ParmPath & "Import/Excel.html?ItemID=" & tCourseID & """,title:[""导入课程数据"",""font-size:16""]" & vbCrlf
	'tmpHtml = tmpHtml & "						,area:[""700px"", ""430px""], maxmin:true" & vbCrlf
	'tmpHtml = tmpHtml & "						,cancel: function(index, layero){ table.reload(""TableList""); }" & vbCrlf
	'tmpHtml = tmpHtml & "					});" & vbCrlf
	tmpHtml = tmpHtml & "					break;" & vbCrlf
	...
	tmpHtml = tmpHtml & "						$.getJSON(""" & ParmPath & "Course/Delete.html"",{ItemID:" & tCourseID & ",ID:arrID.join()}, function(reJson){" & vbCrlf
	tmpHtml = tmpHtml & "							layer.msg(reJson.reMessge,{title:""删除结果"",btn:""关闭"",time:0},function(){ table.reload(""TableList""); });" & vbCrlf
	tmpHtml = tmpHtml & "						});" & vbCrlf
	tmpHtml = tmpHtml & "					});" & vbCrlf
	tmpHtml = tmpHtml & "					break;" & vbCrlf
	tmpHtml = tmpHtml & "				case ""refresh"":table.reload(""TableList"");break;" & vbCrlf
	tmpHtml = tmpHtml & "			};" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf
...
Sub getList()
	...
	Dim tWord : tWord = Trim(ReplaceBadChar(Request("soTeacher")))
	tWord = Replace(tWord, chr(9), "")

	Dim tSoVA8 : tSoVA8 = Trim(ReplaceBadChar(Request("VA9")))
	Dim tTitle : tTitle = Trim(ReplaceBadChar(Request("soWord")))
	Dim soStuType : soStuType = Trim(ReplaceBadChar(Request("StudentType")))

	Dim isMeLoad : isMeLoad = HR_CBool(Request("IsMeload"))					'是否查看本人上传
	Dim soPass : soPass = HR_CBool(Request("IsPass"))						'查看未审核
	Dim soAffirm : soAffirm = HR_CBool(Request("IsAffirm"))					'是否查看已确认
	Dim soRetreat : soRetreat = HR_CBool(Request("IsRetreat"))					'是否查看已退回

	Dim soKSDMItem, soKSDM : soKSDM = Trim(ReplaceBadChar(Request("KSMC")))				'搜索科室代码
	Dim soCampus : soCampus = Trim(ReplaceBadChar(Request("Campus")))		'搜索校区
	Dim soUpuser : soUpuser = HR_CLng(Request("upuser"))					'搜索上传者
	If isMeLoad Then soUpuser = UserID
	...
	'----- 取表结构
	Set rsTmp = Conn.Execute("Select Top 1 ClassName,Unit,SheetName,FieldLen,Template From HR_Class Where ClassID=" & tCourseID)
		If Not(rsTmp.BOF And rsTmp.EOF) Then
			tCourse = rsTmp("ClassName")
			tUnit = rsTmp("Unit")
	...
	'初始化年度开关【记得一定删除】
	'Conn.Execute("Update " & tSheetName & " Set scYear=" & scYear & ",scTerm=0")

	If tTempTable = "TempTableA" Or tTempTable = "TempTableC" Or tTempTable = "TempTableD" Or tTempTable = "TempTableE" Then hrDate = True
	If soKSDM <> "" Then soKSDMItem = Trim(GetTeacherYGDMItems(soKSDM, 0))

	Dim sqlWhere : sqlWhere = ""
	If HR_Clng(soKSDM) > 0 And HR_IsNull(soKSDMItem) Then		'搜索科室，科室无员工
		sqlWhere = sqlWhere & " And VA1='0'"
	ElseIf HR_Clng(soKSDM) > 0 And HR_IsNull(soKSDMItem) = False Then		'科室有员工
		sqlWhere = sqlWhere & " And VA1 in(" & soKSDMItem & ")"
	End If
	If tWord <> "" Then
		If HR_Clng(tWord) > 0 Then
			sqlWhere = sqlWhere & " And VA1=" & tWord		'搜索员工工号
		Else
			sqlWhere = sqlWhere & " And VA2 like '%" & tWord & "%'"		'搜索员工姓名
		End If
	End If
	...
	If soUpuser > 0 Then sqlWhere = sqlWhere & " And UserID=" & soUpuser					'查看指定管理员上传
	...
	sqlGet = "Select * From " & tSheetName & " Where scYear=" & tmpYear & " And ItemID=" & tCourseID & sqlWhere
	...
	Set rsGet = Server.CreateObject("ADODB.RecordSet")
		...
		If Not(rsGet.BOF And rsGet.EOF) Then
			...
			Do While Not rsGet.EOF
				...
				tmpData = tmpData & "{""ID"":" & rsGet("ID") & ",""CourseID"":""" & tCourseID & """,""Course"":""" & Trim(tCourse) & """,""StudentType"":""" & Trim(rsGet("StudentType")) & """,""KSMC"":""" & tKSMC & """"
				tmpData = tmpData & ",""PRZC"":""" & Trim(tPRZC) & """,""YGXB"":""" & arrSex(HR_Clng(tYGXB)) & """,""Time"":""" & Trim(tmpTime) & """"
				tmpData = tmpData & ",""VA0"":""" & HR_Clng(rsGet("VA0")) & """,""VA1"":""" & Trim(rsGet("VA1")) & """,""VA2"":""" & tVA2 & """,""VA3"":""" & FormatNumber(tVA3, 2, -1) & """,""VA4"":""" & tVA4 & """"
				...
				If HR_CLng(rsGet("State")) = 1 Then strAffirm = "已确认"
				If HR_CBool(rsGet("Passed")) Then isAffirm = "false"		'若已经审核，则不能确认

				'是否退回
				tRetreat = "" : isBack = "true"
				If HR_Clng(rsGet("Retreat")) = 1 Then tRetreat = "是" : isBack = "false"
				If HR_CBool(rsGet("Passed")) Then isBack = "false"			'若已经审核，则不能退回

				If UserRank = 1 Then
					If HR_Clng(rsGet("UserID")) = UserID Then
						If HR_CBool(rsGet("Passed")) = False Then
							isPass = "true"
							isEdit = "true" : isDel = "true"
							passStyle = "pass-true"
						End If
					ElseIf HR_Clng(rsGet("UserID")) = 0 Then
						If FoundInArr(UserStuType, Trim(rsGet("StudentType")), ",") Then
							isPass = "true"
							If HR_CBool(rsGet("Passed")) = False Then
								isEdit = "true" : isDel = "true"
								passStyle = "pass-true"
							End If
						ElseIf HR_CBool(rsGet("Passed")) = False Then
							isPass = "true"
							passStyle = "pass-true"
						End If
					End If
				ElseIf UserRank > 1 Then
					isPass = "true"
					If HR_CBool(rsGet("Passed")) = False Then
						isEdit = "true" : isDel = "true"
						passStyle = "pass-true"
				...
				tmpData = tmpData & ",""isEdit"":" & isEdit & ",""isDel"":" & isDel & ",""isPass"":" & isPass & ",""isBack"":" & isBack & ",""isMeLoad"":""" & isMeLoad & """,""isAffirm"":" & isAffirm
...
Sub EditBody()
	...
	tmpHtml = tmpHtml & "<fieldset class=""layui-elem-field layui-field-title""><legend>" & SubButTxt & "课程【" & tItemName & "】</legend></fieldset>"
	tmpHtml = tmpHtml & "<form class=""layui-form layui-form-pane"" id=""FloatForm"" name=""FloatForm"" lay-filter=""FloatForm"" action="""">" & vbCrlf
	tmpHtml = tmpHtml & "<div class=""hr-fix"" id=""xlsBox"">" & vbCrlf
	...
	tmpHtml = tmpHtml & "<input type=""text"" name=""YGXM"" id=""ygxm"" value=""" & arrField(2) & """ lay-verify=""required"" autocomplete=""on"" title=""请点击选择按钮查找教师"" class=""layui-input txt1"" readonly>"
	tmpHtml = tmpHtml & "		</div>" & vbCrlf
	tmpHtml = tmpHtml & "		<div class=""layui-inline""><span class=""layui-btn getBtn"" data-code=""ygdm"" data-name=""ygxm"">查找</span><span class=""tips"">请输入关键字搜索教师，必填项</span></div>" & vbCrlf
	tmpHtml = tmpHtml & "	</div>" & vbCrlf
	tmpHtml = tmpHtml & "	<div class=""layui-form-item"">" & vbCrlf
	tmpHtml = tmpHtml & "		<label class=""layui-form-label"">工　　号：</label>" & vbCrlf
	tmpHtml = tmpHtml & "		<div class=""layui-input-inline""><input type=""text"" name=""YGDM"" id=""ygdm"" lay-verify=""required"" value=""" & arrField(1) & """ class=""layui-input txt1"" readonly></div>" & vbCrlf
	tmpHtml = tmpHtml & "	</div>" & vbCrlf
	If strStuType <> "" Then
	...
	tmpHtml = tmpHtml & "	<div class=""layui-form-item"">" & vbCrlf
	tmpHtml = tmpHtml & "		<div class=""layui-inline""><label class=""layui-form-label"">" & arrHeadKey(3) & "</label>" & vbCrlf
	...
	tmpHtml = tmpHtml & "		</div>" & vbCrlf
	tmpHtml = tmpHtml & "		<div class=""layui-inline""><label class=""layui-form-label"">" & arrHeadKey(4) & "</label>" & vbCrlf
	If dateType Then
	...
	ElseIf tTemplate = "TempTableA" Then
		...
		tmpHtml = tmpHtml & "			<div class=""layui-input-inline""><input type=""text"" name=""VA5"" value=""" & arrField(5) & """ lay-verify=""required"" autocomplete=""on"" class=""layui-input""></div>" & vbCrlf
		tmpHtml = tmpHtml & "		</div>" & vbCrlf
		...
		tmpHtml = tmpHtml & "		<label class=""layui-form-label"">" & arrHeadKey(7) & "</label>" & vbCrlf
		tmpHtml = tmpHtml & "		<div class=""layui-input-inline width_80""><input type=""text"" name=""VA7"" id=""VA7"" value=""" & arrField(7) & """ lay-verify=""required"" autocomplete=""on"" class=""layui-input txt1"" readonly></div>"
	...
	tmpHtml = tmpHtml & "			,multiple: true,acceptMime:'application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/pdf,application/x-zip-compressed',exts:'zip|rar|jpg|jpeg|png|doc|docx|xls|xlsx|gif|pdf|txt'" & vbCrlf
...
Sub SaveEdit()
	...
	Select Case tTempTable
		...
		Case "TempTableF"
			...
			End If
		Case "TempTableG"
			sqlChk = sqlChk & " And VA5='" & Trim(Request("VA5")) & "' And VA6='" & Trim(Request("VA6")) & "'"		'判断学期、项目名称、级别
			If HR_IsNull(Request("VA5")) Or HR_IsNull(Request("VA6")) Then
	...
	ErrMsg = tYGXM & "老师，系统" & SubButTxt & "了您的课程业绩需要您审阅，考核项目：" & tItemName & "[序号：" & tPXXH & "]。"
	Call SendMessage(1, tItemID, TempID, tYGDM, "系统" & SubButTxt & "了您的课程业绩，请审阅", ErrMsg, "")

	tmpJson = "{""Return"":true,""Err"":0,""reMessge"":""教师" & Trim(tYGXM) & " 课程" & SubButTxt & "成功！"",""ReStr"":""操作成功！""}"
	Response.Write tmpJson
End Sub
...
Function GetUploadUerOption()			'上传者Option
	Dim strFun, rsFun
	Set rsFun = Server.CreateObject("ADODB.RecordSet")
		rsFun.open "Select YGXM,UserID From HR_User Where UserID>0", Conn, 1, 1
		If Not(rsFun.BOF And rsFun.EOF) Then
			Do While Not rsFun.EOF
				strFun = strFun & "<option value=""" & rsFun("UserID") & """>" & rsFun("YGXM") & "</option>"
				rsFun.MoveNext
			Loop
		End If
		rsFun.Close
	Set rsFun = Nothing 
	GetUploadUerOption = strFun
End Function
%>