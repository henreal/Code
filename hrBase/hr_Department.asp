...
Dim scriptCtrl : SiteTitle = "科室管理"
Dim SubButTxt : SubButTxt = "所有科室"
...
Sub MainBody()
	...
	strHtml = vbCrlf & "	<script type=""text/javascript"" src=""" & InstallDir & "Static/js/jquery.nicescroll.min.js""></script>" & vbCrlf
	strHeadHtml = Replace(strHeadHtml, "[@HeadScript]", strHtml)
	Response.Write strHeadHtml
	strHtml = "<a href=""" & ParmPath & "Department/List.html"">" & SiteTitle & "</a><a><cite>" & SubButTxt & "</cite></a>"
	...
	Response.Write "	<div class=""layui-form soBox""><div class=""layui-inline"">搜索科室：</div><div class=""layui-inline hr-width_100""><input class=""layui-input"" name=""SearchWord"" id=""SearchWord"" autocomplete=""off"" /></div>" & vbCrlf
	Response.Write "		<div class=""layui-inline searchBtn""><button class=""layui-btn layui-btn-normal hr-btn_deon"" data-type=""reload"" id=""SearchBtn""><i class=""layui-icon"">&#xe615;</i>搜索</button></div>" & vbCrlf
	Response.Write "		<div class=""layui-btn-group searchBtn""><button class=""layui-btn layui-btn-normal"" data-type=""delete"" id=""BatchDel"" title=""批量删除""><i class=""layui-icon"">&#xe640;</i></button>" & vbCrlf
	Response.Write "			<button class=""layui-btn layui-btn-normal"" data-type=""addNew"" id=""addNew"" title=""新增一级科室""><i class=""layui-icon"">&#xe654;</i></button>" & vbCrlf
	Response.Write "			<button class=""layui-btn layui-btn-normal"" data-type=""import"" id=""import"" title=""科室导入""><i class=""hr-icon"">&#xecb8;</i>导入</button>" & vbCrlf
	...
	Response.Write "	<table class=""layui-table"" lay-data=""{url:'" & ParmPath & "Department/AllData.html',height:'full-130',page:true,toolbar:'#tabbtn',limit:20,limits:[10,15,20,30,50,100,1000],id:'TableList'}"" lay-filter=""TableList"">"
	...
	strHtml = strHtml & "				url:""" & ParmPath & "Department/AllData.html"",where: {SearchWord:key1}" & vbCrlf
	strHtml = strHtml & "			});" & vbCrlf
	strHtml = strHtml & "		});" & vbCrlf

	strHtml = strHtml & "		$(""#sort"").on(""click"", function(){" & vbCrlf
	strHtml = strHtml & "		layer.load();" & vbCrlf
	strHtml = strHtml & "			$.getJSON(""" & ParmPath & "Department/DeptSort.html"", function(strForm){" & vbCrlf
	...
	strHtml = strHtml & "			$.get(""" & ParmPath & "Department/Import.html"", function(strForm){" & vbCrlf
	strHtml = strHtml & "				layer.open({type:1,content:strForm,title:[""导入科室数据"",""font-size:16""],area:[""90%"", ""90%""],maxmin:true});" & vbCrlf
	strHtml = strHtml & "				form.on(""submit(ImportPost)"", function(PostData){" & vbCrlf
	strHtml = strHtml & "					$.getJSON(""" & ParmPath & "Department/ImportPost.html"", function(strForm){" & vbCrlf
	...
	strHtml = strHtml & "			$.get(""" & ParmPath & "Department/AddNew.html?AddNew=True"", function(strForm){" & vbCrlf
	strHtml = strHtml & "				layer.open({type:1,content:strForm,title:[""添加一级科室"",""font-size:16""],area:[""700px"", ""80%""],maxmin:true });" & vbCrlf
	strHtml = strHtml & "				form.render();layer.close(loadTips);" & vbCrlf
	strHtml = strHtml & "				form.on(""submit(SubPost)"", function(PostData){" & vbCrlf
	strHtml = strHtml & "					$.post(""" & ParmPath & "Department/SaveForm.html"",PostData.field, function(result){" & vbCrlf
	...
	strHtml = strHtml & "				}" & vbCrlf
	strHtml = strHtml & "				$.getJSON(""" & ParmPath & "Department/Delete.html?ID="" + arrID, function(strForm){" & vbCrlf
	...
	strHtml = strHtml & "				location.href=""../Department/Index/3.html?ItemID="" + data.DepartID;" & vbCrlf
	strHtml = strHtml & "			}else if(obj.event === ""detail""){" & vbCrlf
	strHtml = strHtml & "				$.get(""" & ParmPath & "Department/Preview.html?ID="" + data.DepartID, function(strForm){" & vbCrlf
	strHtml = strHtml & "					layer.open({type:1,content:strForm,title:[""查看科室信息"",""font-size:16""],area:[""700px"", ""80%""],maxmin:true});" & vbCrlf
	strHtml = strHtml & "					form.render();" & vbCrlf
	strHtml = strHtml & "					$("".layui-layer-content"").niceScroll();" & vbCrlf
	strHtml = strHtml & "				});" & vbCrlf
	strHtml = strHtml & "			}else if(obj.event === ""add""||obj.event === ""edit""){" & vbCrlf
	strHtml = strHtml & "				$.get(""" & ParmPath & "Department/Edit.html"",{ID:data.DepartID,ParentID:data.KSDM,Eve:obj.event}, function(strForm){" & vbCrlf
	...
	strHtml = strHtml & "					$.getJSON(""" & ParmPath & "Department/Delete.html?ID="" + data.DepartID, function(reData){" & vbCrlf
	strHtml = strHtml & "						if(reData.Return){;" & vbCrlf
...
Sub EditBody()
	...
	Dim tmpHtml, tKSDM, tKSMC, tPYDM, tPXBH, tKSDD, tSJKS, tKSWM, tDepth
	SubButTxt = "添加"
	...
	tmpHtml = tmpHtml & "<legend>" & SubButTxt & opTitle & "</legend>"
	...
	tmpHtml = tmpHtml & "<div class=""layui-input-block""><button class=""layui-btn"" lay-submit lay-filter=""SubPost"">" & SubButTxt & "</button><button type=""reset"" class=""layui-btn layui-btn-primary"">重置</button></div>"
	tmpHtml = tmpHtml & "</div>"
...
Sub ImportData()
	...
	tmpHtml = "<fieldset class=""layui-elem-field site-demo-button"" style=""margin:10px;"">" & vbCrlf
	tmpHtml = tmpHtml & "<legend>远程接口数据【科室】</legend>"
...
' 子节点数组取值：jsonObj.reData(0).KSMC
	...
	tmpHtml = tmpHtml & "<form class=""layui-form layui-form-pane"" id=""ImportForm"" name=""ImportForm"" action="""">" & vbCrlf
	tmpHtml = tmpHtml & "<div class=""layer-hr-box"" id=""ImportBox"">" & vbCrlf
	tmpHtml = tmpHtml & "	<div class=""layui-form-item"">" & vbCrlf
	tmpHtml = tmpHtml & "		<div class=""layui-input-block""><button class=""layui-btn"" lay-submit lay-filter=""ImportPost"">导入</button><button type=""reset"" class=""layui-btn layui-btn-primary"">重置</button></div>" & vbCrlf
	tmpHtml = tmpHtml & "	</div>" & vbCrlf
	tmpHtml = tmpHtml & "</div>" & vbCrlf
