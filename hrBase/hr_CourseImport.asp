...
Sub MainBody()
	...
	tmpHtml = "<a href=""" & ParmPath & "CourseImport.html?ItemID=" & tCourseID & """>" & SiteTitle & "</a><a><cite>" & tCourse & "</cite></a>"
	...
	Response.Write "	<fieldset class=""layui-elem-field layui-field-title"" style=""margin-top:1px;color:#f30""><legend>" & tCourse & " 【我导入的课程】</legend></fieldset>" & vbCrlf
	If tChild = 0 Then
		...
		Response.Write "	<table class=""layui-table"" lay-data=""{url:'" & ParmPath & "CourseImport/AllData.html?CourseID=" & tCourseID & "&SearchWord=" & tSearchWord & "',height:'full-220',page:true,toolbar: '#tabbtn',limit:20,limits:[10,20,50,100,200,500,1000,2000],text:{none:'暂时未找到相关课程'},id:'TableList'}"" lay-filter=""TableList"">"
		...
		If IsImport Then		'导入
			Response.Write "				<button type=""button"" class=""layui-btn layui-btn-normal"" lay-event=""import"" id=""import"" title=""课程导入""><i class=""hr-icon"">&#xecb8;</i>导入</button>" & vbCrlf
		Else
	...
	tmpHtml = tmpHtml & "					url:'" & ParmPath & "CourseImport/AllData.html', where:postStr" & vbCrlf
	...
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "CourseImport/1knopass.html"",{ItemID:" & tCourseID & "}, function(strForm){" & vbCrlf
	...
	tmpHtml = tmpHtml & "						arrID = arrID + checkStatus.data[i].ID;" & vbCrlf
	tmpHtml = tmpHtml & "					}" & vbCrlf
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "CourseImport/Delete.html"",{ItemID:" & tCourseID & ",ID:arrID}, function(strForm){" & vbCrlf
	...
	tmpHtml = tmpHtml & "			var tID = $(this).data(""id"");location.href=""" & ParmPath & "CourseImport.html?ItemID="" + tID;" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf
	...
	tmpHtml = tmpHtml & "			$.get(""" & ParmPath & "CourseImport/ViewItem.html"",{ItemID:tID}, function(strForm){" & vbCrlf
	...
	tmpHtml = tmpHtml & "				layer.open({type:2,content:'" & ParmPath & "CourseImport/Preview.html?ItemID='+ data.CourseID +'&ID='+data.ID,title:[""查看课程信息"",""font-size:16""],area:[""700px"", ""80%""],maxmin:true});" & vbCrlf
	tmpHtml = tmpHtml & "			}else if(obj.event === ""add""||obj.event === ""edit""){" & vbCrlf
	tmpHtml = tmpHtml & "				layer.open({type:2,content:'" & ParmPath & "CourseImport/Edit.html?ItemID=' + data.CourseID + '&ID='+data.ID,title:[""编辑课程"",""font-size:16""],area:[""760px"", ""90%""],maxmin:true});" & vbCrlf
	...
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "CourseImport/Delete.html"",{ItemID:data.CourseID,ID:data.ID}, function(reData){" & vbCrlf
	...
	tmpHtml = tmpHtml & "				$.getJSON(""" & ParmPath & "CourseImport/Passed.html"",{ID:data.ID,ItemID:data.CourseID,type:1}, function(reData){" & vbCrlf
	...
	tmpHtml = tmpHtml & "					layer.open({type:2, content:'" & ParmPath & "CourseImport/AddNew.html?ItemID=" & tCourseID & "',title:[""添加新课程"",""font-size:16""],area:[""760px"",""90%""],maxmin:true});" & vbCrlf
	...
	tmpHtml = tmpHtml & "						$.getJSON(""" & ParmPath & "CourseImport/Delete.html"",{ItemID:" & tCourseID & ",ID:arrID.join()}, function(reJson){" & vbCrlf
...
Sub getList()
	...
	sqlWhere = sqlWhere & " And UserID=" & soUpuser					'查看指定管理员上传
	...
	Set rsGet = Server.CreateObject("ADODB.RecordSet")
		...
		If Not(rsGet.BOF And rsGet.EOF) Then
			...
			Do While Not rsGet.EOF
				...
				tmpData = tmpData & ",""isEdit"":" & isEdit & ",""isDel"":" & isDel & ",""isPass"":" & isPass & ",""isBack"":" & isBack & ",""isMeLoad"":""true"",""isAffirm"":" & isAffirm
