...
Select Case Action
	...
	Case "AgreeForm" Call AgreeForm()
	Case Else Response.Write GetErrBody(0)
End Select

Sub MainBody()
	Dim passSwap : passSwap = False		'判断本人是否有审核权限
	...
	Dim layUrl : layUrl = ParmPath & "Swap/jsonList.html"
	...
	tmpHtml = "<a href=""" & ParmPath & "Swap/Index.html"">" & SiteTitle & "</a><a><cite>列表</cite></a>"
	...
	Response.Write "			<div class=""sumbar"">共有<b class=""Count"">0</b>条换课申请</div>" & vbCrlf
	Response.Write "		</div>" & vbCrlf
	...
	tmpHtml = tmpHtml & "			,text:{none:'暂时没有换课申请'},cols: [[" & vbCrlf				'设置表头
	...
	tmpHtml = tmpHtml & "				,{field:'ApplyTime',title:'申请时间',align:'center',sort:true,width:150}" & vbCrlf
	tmpHtml = tmpHtml & "				,{field:'Reason',title:'换课原因',minWidth:300,event:'viewIntro',style:'cursor: pointer;'}" & vbCrlf
	tmpHtml = tmpHtml & "				,{field:'CourseDate',title:'申请课程',width:200}" & vbCrlf	
	tmpHtml = tmpHtml & "				,{field:'Course',title:'课程名称',width:130}" & vbCrlf
	tmpHtml = tmpHtml & "				,{field:'Replacer',title:'换课教师',width:90}" & vbCrlf
	tmpHtml = tmpHtml & "				,{field:'CourseDate1',title:'换课课程',width:200}" & vbCrlf
	tmpHtml = tmpHtml & "				,{field:'Course1',title:'课程名称',width:130}" & vbCrlf
	...
	tmpHtml = tmpHtml & "					layer.confirm(""确认要删除选中的 "" + data.length + "" 条换课申请？<br />删除后将无法恢复。"",{icon:3, title:[""删除警告"",""background-color:#f30""]},function(index){" & vbCrlf
	tmpHtml = tmpHtml & "						$.getJSON(""" & ParmPath & "Swap/Delete.html"",{ID:arrID.join()}, function(reData){" & vbCrlf
	...
	tmpHtml = tmpHtml & "				layer.confirm(""您确定要删除该换课申请？"",{icon:3,title:[""删除警告"",""background-color:#f30""]},function(index){" & vbCrlf
	tmpHtml = tmpHtml & "					$.getJSON(""" & ParmPath & "Swap/Delete.html"",{ID:data.ID}, function(reData){" & vbCrlf
	...
	tmpHtml = tmpHtml & "				layer.open({type:2, id:""EditWin"", content:'" & ParmPath & "Swap/Details.html?ID='+data.ID,title:[""换课详情""],area:[""900px"", ""92%""]});" & vbCrlf
	tmpHtml = tmpHtml & "			}" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf
	...
	Set rsGet = Server.CreateObject("ADODB.RecordSet")
		...
		If Not(rsGet.BOF And rsGet.EOF) Then
			...
			Do While Not rsGet.EOF
				...
				tmpJson = tmpJson & ",""CourseDate"":""" & FormatDate(rsGet("VA4"), 4) & " 第" & Trim(rsGet("VA7")) & "节"",""Course1"":""" & Trim(rsGet("newVA8")) & """,""CourseDate1"":""" & FormatDate(rsGet("newVA4"), 4) & " 第" & Trim(rsGet("newVA7")) & "节"""
...
Sub Details()
	...
	Response.Write "			<button type=""button"" class=""layui-btn layui-bg-green"" data-event=""agree"" title=""同意""><i class=""hr-icon"">&#xe95f;</i>同意</button>" & vbCrlf
	Response.Write "			<button type=""button"" class=""layui-btn layui-bg-green"" data-event=""refuse"" title=""拒绝""><i class=""hr-icon"">&#xe960;</i>拒绝</button>" & vbCrlf
	...
	Response.Write "		<dl class=""hr-rows""><dt>申请人：</dt><dd>" & tProposer & " [" & tProposerCode & "]</dd></dl>" & vbCrlf
	...
	Response.Write "	</div>" & vbCrlf
	Response.Write "	<fieldset class=""layui-elem-field layui-field-title site-title""><legend>换课教师</legend></fieldset>" & vbCrlf
	Response.Write "	<div class=""hr-swap-items"">" & vbCrlf
	Response.Write "		<dl class=""hr-rows""><dt>换课教师：</dt><dd>" & tReplacer & "　" & tReplacerCode & "</dd></dl>" & vbCrlf
	...
	Response.Write "	</div>" & vbCrlf
	Response.Write "	<fieldset class=""layui-elem-field layui-field-title site-title""><legend>对换课程信息</legend></fieldset>" & vbCrlf
	Response.Write "	<div class=""hr-rows-pan"">" & vbCrlf
	Response.Write "		<div class=""hr-swap-items row2"">" & vbCrlf
	Response.Write "			<h3 class=""hr-swap-tit"">申请人课程信息</h3>" & vbCrlf
	...
	Response.Write "		</div>" & vbCrlf
	Response.Write "		<div class=""hr-swap-items row2"">" & vbCrlf
	Response.Write "			<h3 class=""hr-swap-tit"">换课课程信息</h3>" & vbCrlf
	Response.Write "			<dl class=""hr-rows""><dt>项目名称：</dt><dd>" & tItemName & "</dd></dl>" & vbCrlf
	...
	tmpHtml = tmpHtml & "	layui.use([""table"", ""element""], function(){" & vbCrlf
	tmpHtml = tmpHtml & "		var table = layui.table, element = layui.element;" & vbCrlf
	tmpHtml = tmpHtml & "		$("".passbtn button"").on(""click"",function(){" & vbCrlf
	tmpHtml = tmpHtml & "			var event = $(this).data(""event"");" & vbCrlf
	tmpHtml = tmpHtml & "			if(event==""agree""){" & vbCrlf
	tmpHtml = tmpHtml & "				layer.confirm(""您将同意本次换课申请，确定？？"",{icon: 3, title:""审核提示""},function(index){" & vbCrlf
	tmpHtml = tmpHtml & "					layer.close(layer.index);" & vbCrlf
	tmpHtml = tmpHtml & "					layer.open({type:2, id:""imGrade"", content:""" & ParmPath & "Swap/AgreeForm.html?ID=" & tmpID & """,title:[""下一步：转结教辅"",""font-size:16""],offset:[""20%"", ""15%""],area:[""560px"", ""360px""]});" & vbCrlf
	tmpHtml = tmpHtml & "				});" & vbCrlf
	tmpHtml = tmpHtml & "				" & vbCrlf
	tmpHtml = tmpHtml & "			}" & vbCrlf
	tmpHtml = tmpHtml & "		});" & vbCrlf
...
Sub EditBody()
	...
	tmpHtml = tmpHtml & "			$.post(""" & ParmPath & "Swap/CourseOption.html"", {ItemID:data.value, ygdm:ygdm, CourseID:course}, function(rsStr){" & vbCrlf
	...
	tmpHtml = tmpHtml & "			$.getJSON(""" & ParmPath & "Swap/GetCourseData.html"", {ItemID:itemid, ygdm:ygdm, CourseID:data.value}, function(rsStr){" & vbCrlf
...
Sub AgreeForm()
	...
	Response.Write "			<legend>请选择教辅</legend>" & vbCrlf
	Response.Write "			<div class=""select-wrap"">" & vbCrlf
	Response.Write "				" & vbCrlf
	Response.Write "			</div>" & vbCrlf
	Response.Write "		</fieldset>" & vbCrlf
	Response.Write "		<div class=""hr-pop-fix"">" & vbCrlf
	Response.Write "		</div>" & vbCrlf
	Response.Write "		<input name=""ID"" type=""hidden"" value=""" & tmpID & """>" & vbCrlf
	Response.Write "		<div class=""hr-pop-fix"">" & vbCrlf
	Response.Write "			<div class=""hr-grids hr-btn-group"">" & vbCrlf
	Response.Write "				<em><button type=""button"" class=""layui-btn hr-btn_deon"" id=""EditPost"" data-event=""EditPost"" lay-submit title=""保存""><i class=""hr-icon"">&#xf0c7;</i>确定</button></em>" & vbCrlf
	Response.Write "				<em><button type=""button"" class=""layui-btn layui-btn-primary"" id=""refresh"" data-event=""refresh"" title=""刷新""><i class=""hr-icon"">&#xf343;</i></button></em>" & vbCrlf
	Response.Write "			</div>" & vbCrlf
	Response.Write "		</div>" & vbCrlf
	Response.Write "	</form>" & vbCrlf
